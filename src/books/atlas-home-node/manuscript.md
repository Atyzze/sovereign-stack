...
- **It leaks where secrets leak.** Unlike a private key, a URL token is sent on every request and comes to rest in browser history, bookmarks, and your server logs, the exact place your monitoring (Part 7) piles it up in plain text. Treat those logs as sensitive as the key, and rotate it.
...
{\footnotesize\itshape One agent is a loop; Part 8 is a few of them, each small, each fenced, all kept alive by systemd, with a human hand on the one lever that spends money.}
\end{center}
```
